...
namespace HS_QuanLyBaoTri
{
    public partial class CustomAppointmentForm1 : DevExpress.XtraEditors.XtraForm, IDXManagerPopupMenu
    {
        ...
        public string _Vocher_Maintenance;
        string _Priority;
        string _Human;
        public string _IDScheduleBig;
        public virtual void LoadFormData(Appointment appointment)
        {

            if (appointment.CustomFields["IDEquipment"] == null)
                txtThietBi.Text = "";
            else
            {
                _IDEquipment = appointment.CustomFields["IDEquipment"].ToString();
            ...
            if (appointment.CustomFields["IDScheduleBig"] == null)
                _IDScheduleBig = "";
            else
            {
                _IDScheduleBig = appointment.CustomFields["IDScheduleBig"].ToString();
                this.data_Get_VatTuCanBaoTriTableAdapter.Fill(dataSet_Get_VatTuBaoTri.Data_Get_VatTuCanBaoTri, _IDScheduleBig, true);
            }
        }
        public virtual bool SaveFormData(Appointment appointment)
        {
            appointment.CustomFields["IDEquipment"] = txtThietBi.EditValue.ToString();
            appointment.CustomFields["IDDept"] = _IDDept;
            appointment.CustomFields["Vocher_Maintenance"] = _Vocher_Maintenance;
        ...
        {
            if (_IDEquipment == appointment.CustomFields["IDEquipment"].ToString() && _Priority == appointment.CustomFields["Priority"].ToString() && _Human == appointment.CustomFields["PerformanceHuman"].ToString())
                return false;
            else
                return true;
        }
        ...
        private void gridView3_ValidateRow(object sender, DevExpress.XtraGrid.Views.Base.ValidateRowEventArgs e)
        {
            ...
            try
            {
                ...
                cmd.Parameters.AddWithValue("@IDAppointments", _IDScheduleBig);
